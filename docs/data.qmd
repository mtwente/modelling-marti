...
All data is available in `csv` format. For this file format, metadata is provided in `json` files according to the W3C Metadata Vocabulary for Tabular Data [@w3cModelTabularData2022].

### Article Data

In the R workflow, the `metadata` object is used to create a list of `articles` which includes an URL to download the textual data into another R object `text_data`. The resulting `marti_corpus` contains the full articles, metadata on those articles and information on Marti's work status at the time of publication for each article (@fig-datamodel).
